 ...
 * @return {Promise<Spotify.Info | undefined>} - Data extracted from the URI.
 * @throws {SPOTIFY_URI_PARSE_ERROR} - Unable to parse the given URI.
 * @throws {SPOTIFY_UNSUPPORTED_URI_TYPE} - Given URI is not supported.
 * @throws {SPOTIFY_REQUEST_FAILED} - Failed to send a request to Spotify.
 *
 * @example
 *
				...
				album: t.album
					? {
							...
							artists: t.album.artists
								? t.album.artists.map((a: Spotify.Artist) => {
										return {
											id: a.id,
											name: a.name,
											uri: a.uri,
